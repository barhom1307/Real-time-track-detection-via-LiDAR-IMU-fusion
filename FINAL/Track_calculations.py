...
def extrapolate_points(xyz_cones, imu_data, time_stamp):
    imu_timestamp = imu_data[-1]
    lidar_timestamp = float(time_stamp)
    vec_len = len(xyz_cones)
    # time_vec = np.linspace(lidar_timestamp, imu_timestamp, vec_len).astype(float)
    return imu_timestamp-lidar_timestamp
