...
class vlp16_decoder:
    ...
    '''
    Abstract:   plot_lidar_cones func used to plot XY lidar frame (Z=0) frame.
                the plot will include full 180 degree front view, and on top on it a scatter of the cones detected in red.
    Parameters: fig- figure which created in main_t
                velo_frame- lidar full frame.
                xyz_cones- cones X & Y values.
                timeStamp- frame's time_stamp in order to attach it to plot's title.
    Returns:    None.
    '''
    def plot_lidar_cones(self, fig, velo_frame, xyz_cones, timeStamp):
        axes_str = ['X', 'Y', 'Z']
        axes_limits = [
            [-3, 3],  # X axis range
            [0, 12],  # Y axis range
