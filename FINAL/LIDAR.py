...
class vlp16_online_feeder:
    def __init__(self, lidar_pkl, FOV, addr = ("0.0.0.0", 2368)):
        self.addr = addr
        self.FOV = FOV
    ...
    '''
    def recv_worker(self, queue, FOV, lidar_pkl):
        self.packets_list = []
    ...
    def run(self):
        self.queue = Queue()
        self.proc = Process(target=self.recv_worker, args=(self.queue, self.FOV, self.lidar_pkl),name='LIDAR_proc')
        self.proc.start()

    '''
    Abstract:   get_packet func used by decoder class to get packets one at a time in order to have full frame of lidar.
    Parameters: None
...
class vlp16_offline_feeder:
    ...
    Returns:    pickle raw_packet (binary), and time_stamp of the packet (float).
                if there is no packets returns None.
    '''
    def get_packet(self):
...
class vlp16_decoder:
    ...
    def get_full_frame(self):
        ...
        # continuing requesting packets till we have 360 deg farme or till we have reached the stop angle
        while total_angle < 360:
            ...
            if self.prev_az >= STOP_ANGLE-2 and self.prev_az <= STOP_ANGLE+2:
                 break
        decoded_frame = np.concatenate(packets_data_list)
